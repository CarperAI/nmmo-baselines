...
class Config:
    ...
    train_num_steps = 10_000_000  # Number of steps to train  TODO 10_000_000
    ...
    run_name = f"{'test'}_{seed}_{time.strftime('%Y%m%d_%H%M%S')}"  # Run name
    ...
    wandb_project = "nips2023-nmmo"  # WandB project name
    wandb_entity = "mori42"  # WandB entity name

    # PPO Args
    bptt_horizon = 8  # Train on this number of steps of a rollout at a time. Used to reduce GPU memory.
    ppo_training_batch_size = 128  # Number of rows in a training batch  TODO 有没有进行层归一化
    ...
    heal_bonus_weight = 0.02
    meander_bonus_weight = 0.01
    explore_bonus_weight = 0.5
    health_bonus_weight = 0.005
    spawn_immunity = 20

    # New rewards added
    eat_poultice_weight = 0.03
    eat_food_weight = 0.01
    starved_or_dehydrated = -0.001
    deal_bonus_weight = -1   # It has not actually taken effect
