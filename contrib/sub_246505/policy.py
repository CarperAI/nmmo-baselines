...
class Baseline(pufferlib.models.Policy):
  def __init__(self, env, input_size=256, hidden_size=256, task_size=4096):
    ...
    self.lstm1 = torch.nn.LSTMCell(input_size=hidden_size, hidden_size=hidden_size)
    self.lstm2 = torch.nn.LSTMCell(input_size=hidden_size, hidden_size=hidden_size)
    self.state = None
  ...
  def encode_observations(self, flat_observations):
    ...
    env_outputs["ActionTargets"]["Give"]["InventoryItem"][:, :-1] = 0
    env_outputs["ActionTargets"]["Give"]["Target"][:, :-1] = 0
    env_outputs["ActionTargets"]["GiveGold"]["Target"][:, :-1] = 0
    env_outputs["ActionTargets"]["GiveGold"]["Price"][:, 1:] = 0

    dangerous_npc_idxs = torch.where(env_outputs["Entity"][:, :, 1] == 2)
    env_outputs["ActionTargets"]["Attack"]["Target"][dangerous_npc_idxs] = 0

    if self.state is None:
      self.state = [torch.zeros_like(my_agent).unsqueeze(0).repeat(2, 1, 1), torch.zeros_like(my_agent).unsqueeze(0).repeat(2, 1, 1)]
    reset_idxs = torch.where(env_outputs["Entity"][:, :, 5].max(dim=-1)[0] == 0)[0]
    self.state[0][:, reset_idxs, :] = 0
    self.state[1][:, reset_idxs, :] = 0
    next_state = [torch.zeros_like(self.state[0]), torch.zeros_like(self.state[1])]
    for i in range(2):
      lstm = getattr(self, f"lstm{i + 1}")
      next_state[0][i], next_state[1][i] = lstm(obs, [self.state[0][i], self.state[1][i]])
      obs = F.relu(next_state[0][i])
...
class PlayerEncoder(torch.nn.Module):
  def __init__(self, input_size, hidden_size):
    ...
    # self.attr_name_to_col = {'id': 0, 'npc_type': 1, 'row': 2, 'col': 3, 'damage': 4, 'time_alive': 5, 'freeze': 6, 'item_level': 7, 'attacker_id': 8, 'latest_combat_tick': 9, 'message': 10, 'gold': 11, 'health': 12, 'food': 13, 'water': 14, 'melee_level': 15, 'melee_exp': 16, 'range_level': 17, 'range_exp': 18, 'mage_level': 19, 'mage_exp': 20, 'fishing_level': 21, 'fishing_exp': 22, 'herbalism_level': 23, 'herbalism_exp': 24, 'prospecting_level': 25, 'prospecting_exp': 26, 'carving_level': 27, 'carving_exp': 28, 'alchemy_level': 29, 'alchemy_exp': 30}
  ...
  def forward(self, agents, my_id):
    ...
    agent_embeddings = self.embedding((agents[:, :, self.embedding_idx].long() + 256).clip(0, 511)).reshape(batch, agent, -1)
    agent_embeddings = torch.cat((agent_embeddings, agents[:, :, self.no_embedding_idx]), dim=-1)
...
class TaskEncoder(torch.nn.Module):
  ...
  def forward(self, task):
    return F.relu(self.norm(self.fc(task.clone())))
