...
class PolicySelector():
  ...
  def select_policies(self, policies: Dict[str, PolicyRecord]) -> List[PolicyRecord]:
    ...
    if len(available_names) < self._num:
      return []
    if len(available_names) > 10 * self._num:
        available_names = available_names[-10 * self._num:]
    selected_names = np.random.choice(available_names, self._num, replace=False).tolist()
...
class CleanPuffeRL:
    ...
    def __post_init__(self, *args, **kwargs):
        ...
        ### Allocate Storage
        next_obs, next_done, lstm_states, next_lstm_state = [], [], [], []
        for i, envs in enumerate(self.buffers):
            ...
            if is_recurrent:
                next_lstm_state.append(
                    [
                        torch.zeros(2, self.num_envs * self.num_agents, 256).to(self.device),
                        torch.zeros(2, self.num_envs * self.num_agents, 256).to(self.device),
                    ]
                )
            else:
        ...
        if is_recurrent:
            lstm_states = [torch.zeros(2, self.batch_size + 1, 256).to(self.device),
                           torch.zeros(2, self.batch_size + 1, 256).to(self.device),
                           ]
        else:
            lstm_states = None
        ...
        if self.wandb_entity is not None:
            ...
            wandb.init(
                ...
                mode="offline" if self.wandb_extra_data.wandb_offline else "online",
    ...
    def evaluate(self, show_progress=True):
        ...
        print(f"Number of policies in the policy pool: {len(self.policy_pool._policies)}")
        ...
        while True:
            ...
            i = self.policy_pool.update_scores(i, "completed_task_count")
            ...
            with torch.no_grad():
                if is_recurrent:
                    self.agent.policy.state = data.next_lstm_state[buf]
                    (
                        actions,
                        logprob,
                        value,
            ...
            for idx in np.where(alive_mask)[0]:
                if ptr == self.batch_size + 1:
                    if is_recurrent:
                        data.next_lstm_state[buf] = self.agent.policy.state
                    break
                ...
                if is_recurrent:
                    data.lstm_states[0][:, ptr] = data.next_lstm_state[buf][0][:, idx]
                    data.lstm_states[1][:, ptr] = data.next_lstm_state[buf][1][:, idx]
        ...
        if self.policy_pool.scores and self.policy_ranker is not None:
          try:
            ...
            print("Error occurred when updating scores, skipping")
          self.policy_pool.scores = {}
    ...
    def train(
        ...
        b_lstm_states = [data.lstm_states[0][:, b_idxs], data.lstm_states[1][:, b_idxs]]
        ...
        for epoch in range(update_epochs):
            for mb in range(num_minibatches):
                ...
                mb_lstm_states = [b_lstm_states[0][:, mb].squeeze(1), b_lstm_states[1][:, mb].squeeze(1)]

                if is_recurrent:
                    self.agent.policy.state = mb_lstm_states
