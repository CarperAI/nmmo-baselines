...
class Baseline(pufferlib.models.Policy):
    def __init__(self, env, input_size=256, hidden_size=256, task_size=4096):
        ...
        self.share_layer = torch.nn.Sequential(
    ...
    def decode_actions(self, hidden, lookup):
        hidden = self.share_layer(hidden)
...
class TileEncoder(torch.nn.Module):
    def __init__(self, input_size):
        ...
        self.tile_conv_4 = torch.nn.Conv2d(16, 8, 3)
        self.tile_fc = torch.nn.Linear(8 * 7 * 7, input_size)
    ...
    def forward(self, tile):
        ...
        tile = F.relu(self.tile_conv_3(tile))
        tile = F.relu(self.tile_conv_4(tile))
...
class PlayerEncoder(torch.nn.Module):
    ...
    def forward(self, agents, my_id):
        ...
        agent_embeddings = F.relu(agent_embeddings)
...
class ItemEncoder(torch.nn.Module):
    ...
    def forward(self, items):
        ...
        item_embeddings = F.relu(self.fc(item_embeddings))
...
class InventoryEncoder(torch.nn.Module):
    ...
    def forward(self, inventory):
        ...
        return F.relu(self.fc(inventory))
...
class MarketEncoder(torch.nn.Module):
    ...
    def forward(self, market):
        return F.relu(self.fc(market).mean(-2))
...
class TaskEncoder(torch.nn.Module):
    ...
    def forward(self, task):
        return F.relu(self.fc(task.clone()))
