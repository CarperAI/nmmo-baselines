...
class Postprocessor(StatPostprocessor):
    ...
    def reward_done_info(self, reward, done, info):
        ...
        if info != {}:
            reward += info["stats"]["event/eat_food"] + 0.2 * info["stats"]["event/drink_water"] + 0.1 * (
                    info["stats"]["event/equip_ammo"] + info["stats"]["event/harvest_weapon"] + info["stats"][
                "event/equip_tool"] + info["stats"]["event/equip_weapon"] + info["stats"][
                        "event/equip_armor"] + info["stats"]["event/harvest_item"] + info["stats"][
                        "event/score_hit"] + 2 * info["stats"]["event/player_kill"])
            if done and not (
                    info["stats"]["cod/attacked"] or info["stats"]["cod/starved"] or info["stats"]["cod/dehydrated"]):
                reward += info["return"]
