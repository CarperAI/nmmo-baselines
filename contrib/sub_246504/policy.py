...
class Block(torch.nn.Module):
    def __init__(self):
      ...
      self.tile_conv_3 = torch.nn.Conv2d(512,512, 3,padding=1)
      self.tile_conv_4 = torch.nn.Conv2d(512, 512, 3,padding=1)
      self.tile_conv_5 = torch.nn.Conv2d(512, 512, 3,padding=1)
      self.tile_conv_6 = torch.nn.Conv2d(512, 512, 3,padding=1)
      self.tile_conv_7 = torch.nn.Conv2d(512, 512, 3,padding=1)
      self.drop=torch.nn.Dropout(p=0.5)
    def forward(self,tile):
      tile=F.gelu(self.tile_conv_3(tile))

      tile=self.max1(tile)

      h1=tile
      tile=F.gelu(self.tile_conv_4(tile))
      tile=F.gelu(self.tile_conv_5(tile))
      tile=tile+h1
      h2=tile
      tile=F.gelu(self.tile_conv_6(tile))
      tile=F.gelu(self.tile_conv_7(tile))
      tile=tile+h2
      tile=self.drop(tile)
      return tile
class TileEncoder(torch.nn.Module):
  def __init__(self, input_size):
    ...
    self.conv1=torch.nn.Conv2d(96,512,kernel_size=1)
    self.block1=Block()
    self.block2=Block()
    self.block3=Block()
    self.tile_fc = torch.nn.Linear(512, input_size)
  ...
  def forward(self, tile):
    ...
    tile=self.block1(tile)
    tile=self.block2(tile)
    tile=self.block3(tile)
    tile=F.gelu(tile)
    tile = tile.contiguous().view(agents, -1)
    tile = F.gelu(self.tile_fc(tile))
...
class PlayerEncoder(torch.nn.Module):
  def __init__(self, input_size, hidden_size):
    ...
    self.agent_conv_1 = torch.nn.Linear(self.entity_dim * 32, 256)
    self.agent_conv_2 = torch.nn.Linear(256, 64)
    self.agent_conv_3 = torch.nn.Linear(64,64)
    self.agent_conv_4 = torch.nn.Linear(64, 256)
    self.agent_conv_5 = torch.nn.Linear(256, self.entity_dim * 32)
    self.my_agent_conv_1 = torch.nn.Linear(self.entity_dim * 32, 256)
    self.my_agent_conv_2 = torch.nn.Linear(256, 64)
    self.my_agent_conv_3 = torch.nn.Linear(64,64)
    self.my_agent_conv_4 = torch.nn.Linear(64, 256)
    self.my_agent_conv_5 = torch.nn.Linear(256, self.entity_dim * 32)
  ...
  def forward(self, agents, my_id):
    ...
    agent_embeddings=F.gelu(self.agent_conv_1(agent_embeddings))

    tmp_agent=agent_embeddings
    agent_embeddings=F.gelu(self.agent_conv_2(agent_embeddings))

    agent_embeddings=F.gelu(self.agent_conv_3(agent_embeddings))
    agent_embeddings=F.gelu(self.agent_conv_4(agent_embeddings))
    agent_embeddings=agent_embeddings+tmp_agent
    agent_embeddings=F.gelu(self.agent_conv_5(agent_embeddings))


    agent_embeddings =self.agent_fc(agent_embeddings)
    my_agent_embeddings=F.gelu(self.my_agent_conv_1(my_agent_embeddings))
    tmp_agent=my_agent_embeddings
    my_agent_embeddings=F.gelu(self.my_agent_conv_2(my_agent_embeddings))

    my_agent_embeddings=F.gelu(self.my_agent_conv_3(my_agent_embeddings))
    my_agent_embeddings=F.gelu(self.my_agent_conv_4(my_agent_embeddings))
    my_agent_embeddings=my_agent_embeddings+tmp_agent
    my_agent_embeddings=F.gelu(self.my_agent_conv_5(my_agent_embeddings))
    my_agent_embeddings = F.gelu(self.my_agent_fc(my_agent_embeddings))
...
class ItemEncoder(torch.nn.Module):
  def __init__(self, input_size, hidden_size):
    ...
    self.linear1=torch.nn.Linear(2 * 32 + 12, 152)
    self.linear2=torch.nn.Linear(152, 64)
    self.linear3=torch.nn.Linear(64, 64)
    self.linear4=torch.nn.Linear(64, 76)
  ...
  def forward(self, items):
    ...
    item_embeddings=F.gelu(self.linear1(item_embeddings))
    item_embeddings=F.gelu(self.linear2(item_embeddings))
    item_embeddings=F.gelu(self.linear3(item_embeddings))
    item_embeddings=F.gelu(self.linear4(item_embeddings))
...
class InventoryEncoder(torch.nn.Module):
  def __init__(self, input_size, hidden_size):
    super().__init__()
    self.norm=torch.nn.BatchNorm1d(12)
...
class TaskEncoder(torch.nn.Module):
  def __init__(self, input_size, hidden_size, task_size):
    ...
    self.tile_conv_1 = torch.nn.Linear(task_size, 2*task_size)
    self.tile_conv_2 = torch.nn.Linear(2*task_size, 64)
    self.tile_conv_3 = torch.nn.Linear(64,64)
    self.tile_conv_4 = torch.nn.Linear(64, 2*task_size)
    self.tile_conv_5 = torch.nn.Linear(2*task_size, task_size)
  ...
  def forward(self, task):
    ...
    tmp_task=task
    task=F.gelu(self.tile_conv_1(task))

    task=F.gelu(self.tile_conv_2(task))
    task=F.gelu(self.tile_conv_3(task))
    task=F.gelu(self.tile_conv_4(task))

    task=F.gelu(self.tile_conv_5(task))
    task=task+tmp_task
    return self.fc(task)
