...
from model.deep_goat.model import MultiHeadAttention

EntityId = EntityState.State.attr_name_to_col["id"]


class DeepGoatPolicy(pufferlib.models.Policy):
  def __init__(self, binding):
      ...
      # :/
      input_size = DeepGoatPolicy.INPUT_SIZE
      hidden_size = DeepGoatPolicy.HIDDEN_SIZE
      ...
      self.task_fc = torch.nn.Linear(1024, input_size)

      self.attention_head = MultiHeadAttention(DeepGoatPolicy.INPUT_SIZE, DeepGoatPolicy.INPUT_SIZE)

      self.proj_fc = torch.nn.Linear(input_size, input_size)
  ...
  def encode_observations(self, env_outputs):
    ...
    emb, attn = self.attention_head(task_emb.unsqueeze(0), tile.unsqueeze(0), tile.unsqueeze(0))
    emb = emb.squeeze(0)

    return self.proj_fc(emb), None
  ...
  def create_policy(num_lstm_layers=1):
    DeepGoatPolicy.INPUT_SIZE = 1024
    if num_lstm_layers == 0:
        DeepGoatPolicy.HIDDEN_SIZE = 1024
        policy = pufferlib.frameworks.cleanrl.make_policy(
        DeepGoatPolicy, recurrent_kwargs={'num_layers': 0})
    return policy
