...
class TestItemHelper(FeaturizerTestTemplate):
  ...
  def test_update_equip_weapons_armors(self):
    env, team_id, entity_helper, item_helper, market_helper = self._create_test_env()
    ...
    env.realm.players[entity_helper.pos_to_agent_id(7)].gold.update(10) # to buy level-0 bottom
    ...
    # check market_helper
    self.assertListEqual(list(market_helper._combat_score),
                         [126, 76, 86, 96, 36, 20, 8, 0])
  ...
  def test_update_equip_tools(self):
    ...
    for pos, (bw, bt) in enumerate(zip(item_helper.best_weapons,
                                       item_helper.best_tools)):
      agent_id = entity_helper.pos_to_agent_id(pos)
      obs_inv = team_obs[agent_id]['Inventory']
      force_use_idx = item_helper.force_use_idx[pos]
  ...
  def test_update_sell_tools_profession(self):
    ...
    env.realm.players[entity_helper.pos_to_agent_id(0)].gold.update(20) # _buy_consumables()
    ...
    env.realm.players[entity_helper.pos_to_agent_id(4)].resources.health.update(20)
    env.realm.players[entity_helper.pos_to_agent_id(4)].gold.update(20)
    env.realm.players[entity_helper.pos_to_agent_id(5)].resources.health.update(20)
